...
\textbf{Step 2}, with $\mu_e$ fixed, the branch parameters $\{\mu_i,\tau_i\}_{i=1}^{n}$ are fitted simultaneously to the eleven cycles with a gradient-free, global particle swarm optimiser~\cite{Optim}. The seed of the random number generator is fixed in the script, so that both the swarm and the sampling below return the same values in every run.
...
\begin{table}[ht]
  \centering
  \caption{Calibrated parameters of VHB 4910 (two-branch model). Coefficients of determination: $R^2=0.995$ (step~1) and $R^2=0.978$ (step~2).}
  ...
  \begin{tabular}{l c r r}
    ...
    $\mu_e$ [kPa]  & $13.4 \pm 0.09$ &  0.7 & 1278.6 \\
    \midrule
    \multicolumn{4}{l}{\textit{Step 2: Non-equilibrium terms (neo-Hookean branches)}} \\
    $\mu_1$ [kPa]  & $11.1 \pm 2.9$  & 26.1 & 45.9 \\
    $\tau_1$ [s]   & $152 \pm 80$    & 52.7 &  5.3 \\
    $\mu_2$ [kPa]  & $30.5 \pm 3.6$  & 11.7 & 19.8 \\
    $\tau_2$ [s]   & $7.07 \pm 2.0$  & 28.0 & 13.0 \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{Results.}
The identification returns two branches with well separated relaxation times, a slow one ($\tau_1=152$\,s) and a fast one ($\tau_2=7.1$\,s), which span the time scales probed by the tests (Table~\ref{tab:calibration}).
...
\begin{figure}[ht]
  ...
  \begin{subfigure}[t]{0.49\textwidth}\centering
    ...
    \caption{$\dot\lambda=0.03\,\mathrm{s^{-1}}$; maximum strains of 50, 100 and 150\,\% ($\lambda_{\max}=1.5,\,2.0,\,2.5$).}
  \end{subfigure}
...
With two branches the ensemble stays within a narrow band around the measured cycle. Adding a third branch leaves the fit unchanged ($R^2=0.977$ in both cases, loss reduction of 0.1\%) but scatters the ensemble over the whole stress range: the relative errors of the relaxation times grow from 28--53\% to 438\% and 1180\%, and the swarm returns a negative shear modulus for one of the branches, outside the admissible range of the model. The additional parameters are therefore not identifiable from the available data, and the two-branch model is the richest one supported by the experiments.
...
The whole section is produced by the script \texttt{calibration/Calibration.jl} of the repository~\cite{Repo} from the CSV data distributed with it, using Julia~1.12, HyperCalibration.jl~v0.0.3 and HyperFEM.jl~v0.0.6~\cite{HyperFEM}. It runs in about a minute on a laptop and returns the values of Table~\ref{tab:calibration} in every run.
...
A square membrane of side $w=200$\,mm and thickness $h=1$\,mm, $\Omega_0=(-w/2,w/2)^2\times(0,h)$, with density $\rho_0=960\,\mathrm{kg/m^3}$, is described by the two-branch model of Table~\ref{tab:calibration}, used as is. Near-incompressibility is enforced by the volumetric term $U(J)=\tfrac{\kappa}{2}(J-1)^2$ with $\kappa=2.5$\,MPa, i.e.\ $\kappa\approx45\,\mu_0$ with $\mu_0=\mu_e+\mu_1+\mu_2=55.0$\,kPa the instantaneous shear modulus; $|J-1|<6\cdot10^{-4}$ throughout the simulation.
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.85\linewidth]{figures/membrane_setup.png}
...
The domain is discretised with $7\times7\times1$ hexahedra with quadratic Lagrangian interpolation (2025 degrees of freedom) and Gauss quadrature exact for polynomials of degree~4.
Time integration is implicit with $\Delta t=1$\,ms up to $t=1$\,s (1000 steps). With the velocity update $\tfrac12(\vv_{n}+\vv_{n+1})=(\uu_{n+1}-\uu_n)/\Delta t$, each step solves
\begin{equation}
...
Figure~\ref{fig:energy} shows that the pulse contributes $W_{\mathrm{ext}}=220\,\mu$J during $t\le0.1$\,s. Afterwards the membrane is isolated: energy is continuously exchanged between its kinetic and elastic parts, while the dissipation grows monotonically, up to $D=9.1\,\mu$J at $t=1$\,s, 4\% of the injected energy. The balance residual $|K+\mathcal{E}+D-W_{\mathrm{ext}}|$ grows linearly in time and stays below $2.9\,\mu$J, i.e.\ below 1.3\% of the injected energy, over the 1000 steps. It is a discretisation error of second order: dividing $\Delta t$ by two and by four reduces it to $0.95$ and $0.19\,\mu$J, while the dissipation converges to $9.23\,\mu$J.

\begin{figure}[htb]
  \centering
  \includegraphics[width=0.75\linewidth]{figures/energy_balance.pdf}
  \caption{Energy balance: accumulated dissipation $D$, kinetic energy $K$ and stored energy $\mathcal{E}$ (stacked areas) versus the external work $W_{\mathrm{ext}}$ (dotted line). The discrepancy of the total energy is a discretisation error.}
  \label{fig:energy}
\end{figure}

\paragraph{Deformation.}
Figure~\ref{fig:snapshots} shows the deformed configuration at selected instants. The pulse bends the membrane around the loaded patch; the flexural wave reaches the free edges right after the pulse ends ($t\approx0.1$\,s), the edges oscillate with a deflection of the order of 7\,mm about the mean plane. The relaxation times of the material are much longer than the simulated time, so this oscillation is barely damped. The whole motion is superposed on the rigid-body translation induced by the impulse of the load, with a mean velocity of 4.5\,cm/s.

\begin{figure}[htb]
  \centering
  \includegraphics[width=0.85\linewidth]{figures/membrane_snapshots.png}
  \caption{Deformed configuration at $t=0$, $0.15$ and $0.3$\,s.}
  \label{fig:snapshots}
\end{figure}

{\sloppy
\paragraph{Reproducibility.}
The example is produced by the script \texttt{simulation/\allowbreak ViscoTimeIntegrator-\allowbreak Neumann.jl} of the repository~\cite{Repo}, with Julia~1.12, HyperFEM.jl~v0.0.6 and Gridap~v0.19. It writes the VTK snapshots and the energy history, and runs in about ten minutes on a laptop; two independent runs reproduce the energy history of Figure~\ref{fig:energy} to the last bit.\par}
