...
% Include figures/<name>.pdf (or .png); draw a labelled placeholder while the file does not exist.
\newcommand{\fig}[3]{%
  ...
  \fbox{\parbox[c][\dimexpr#2*55/100\relax][c]{\dimexpr#2-2\fboxsep-2\fboxrule\relax}{\centering\small
    \texttt{figures/\detokenize{#1}.pdf}\\[4pt]\textcolor{gray}{#3}}}}}}
...
The load is an out-of-plane traction $\bar{\bm{t}}=\bar t_0\,f(t)\,\bm{e}_3$, $\bar t_0=20$\,Pa, applied on the upper and lower faces of the central element, a $w/7\times w/7$ patch (peak resultant force $\approx33$\,mN). The pulse $f(t)$ is triangular: it grows linearly from 0 to 1 at $t=0.05$\,s and vanishes for $t\geq0.1$\,s, after which the membrane evolves freely.

\begin{figure}[ht]
  \centering
  \fig{membrane_setup}{0.75\textwidth}{Geometry, mesh, loaded patch, traction pulse $f(t)$}
...
\begin{figure}[ht]
  \centering
  \fig{energy_balance}{0.75\textwidth}{Stacked $D$, $K$, $\mathcal{E}$ vs.\ $W_{\mathrm{ext}}$; balance residual}
...
\begin{figure}[ht]
  \centering
  \fig{displacement_snapshots}{\textwidth}{Deformed configuration coloured by $u_3$ at selected instants}
  \caption{Deformed configuration coloured by the out-of-plane displacement $u_3$ at $t=\,$[to be completed]\,s.}
  \label{fig:snapshots}
\end{figure}
