...
function solve_problem(data)
  ...
  Vu = TestFESpace(geometry, reffe, conformity=:H1)
  ...
  function residual(t)
    (u, v) -> begin
      ...
      f_mid = data.neumann.value * data.neumann.time(t - Δt/2)
      ∫( ρ₀ * a_mid · v + ∇(v)' ⊙ P_mid )dΩ - ∫( f_mid ⋅ v )dΓ
    end
  end
  ...
  createpvd(outpath) do pvd
    ...
    try
      while time < t_end
        ...
        op = FEOperator(residual(time), jacobian(time), Uu, Vu)
        solve!(uh⁺, solver, op)
...
problem_data = let
  ...
  (; width, thick, speed, hrefinement, xdivisions, prefinement, t_end, Δt, neumann)
end
